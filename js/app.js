...
const showProducts = (products) =>
{
  ...
  for (const product of allProducts) {
    ...
    const averageRating = Math.round(product.rating.rate / 5 * 100);

    div.classList.add("product");
    div.innerHTML = `<div class="single-product">
      <div>
    <img class="product-image m-auto" src=${product.image}></img>
      </div>
      <h3 class="text-4xl mt-10">${product.title.split(" ").slice(0, 3)}</h3>
      <p class="text-2xl py-3">Category: ${product.category}</p>
      <h2 class="text-4xl">Price: $ ${product.price}</h2>
      <div class="stars-outer">
        <div class="stars-inner" style="width:${averageRating}%"></div>
      </div>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="bg-gray-700 py-4 px-4 rounded-md text-3xl w-full my-6">Add to cart</button><br>
      <button id="details-btn" class="bg-pink-600 py-4 px-4 rounded-md text-3xl w-full">Details</button></div>
      `;
